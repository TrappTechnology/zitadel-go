...
ifeq ($(strip $(TAG)),)
	...
	echo "Installing buf v$(BUF_VERSION), protoc-gen-go v$(PROTOC_GEN_GO_VERSION), and protoc-gen-go-grpc v$(PROTOC_GEN_GO_GRPC_VERSION)..."; \
	GOBIN="$$_TMPDIR/bin" go install "github.com/bufbuild/buf/cmd/buf@v$(BUF_VERSION)"; \
